...
<script>
    ...
    export default {
        ...
        created() {
            eventHub.$on('arrange-modules-dialog', this.onShow)
        },
        beforeDestroy() {
            eventHub.$off('arrange-modules-dialog')
        },
        methods: {
            onShow(obj) {
                this.dialog = true
                ...
                if(this.file.ranking) {
                    ...
                    this.rank.forEach((item, index) => {
                        this.list[index] = _.find(this.file.modules, (item) => {
                            return item.id == this.rank[index]
                        })
                    })
                } else {
                    this.list = this.file.modules
                }
            },
            initForm() {
                this.form = new Form({
            ...
            requestArrange() {
                this.form.put(`/api/files/ranking/${this.file.id}`)
                    .then(response => {
                        console.log(response)
                        this.onCancel()
                        this.$emit('arrange-modules-updated')
